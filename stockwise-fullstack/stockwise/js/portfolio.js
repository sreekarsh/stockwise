"use strict";
{
    ...
    const _refreshedByPoll = new Set();
    ...
    function bindEvents() {
        ...
        document.getElementById("btnExchangeKeys").addEventListener("click", () => {
            ...
            if (currentUser.has_coindcx_secret) {
                ...
                if (triggerSyncBtn)
                    triggerSyncBtn.style.display = "inline-block";
            }
            else {
                document.getElementById("dcxKey").value = "";
                document.getElementById("dcxSecret").value = "";
                if (triggerSyncBtn)
                    triggerSyncBtn.style.display = "none";
            }
                ...
                if (key && secret && (key !== "••••••••••••••••" || secret !== "••••••••••••••••")) {
                    ...
                    try {
                        ...
                        if (res.ok)
                            ...
                            console.warn("Failed to save API keys:", await res.json().catch(() => ({})));
                    }
                    catch (e) {
    ...
    function switchChartTab(tab) {
        ...
        if (tab === "alloc") {
            ...
            if (tabAlloc)
                tabAlloc.classList.remove("active");
            if (tabPerf)
    ...
    function initTickerNav() {
        ...
        if (prevBtn) {
            prevBtn.addEventListener("click", () => {
                tickerScrollAmount -= 200;
                if (tickerScrollAmount < 0)
                    tickerScrollAmount = 0;
                track.style.transform = `translateX(-${tickerScrollAmount}px)`;
            });
        }
    }
    ...
    async function fetchLivePrices() {
        ...
        try {
            ...
            const [dcxResult, cgResult, cgHoldingsResult] = await Promise.allSettled([
                ...
                fetch("/api/markets?per_page=250&vs_currency=inr&price_change_percentage=1h,24h,7d").then((r) => r.ok ? r.json() : null),
                targetIds.length > 0
                    ? fetch(`/api/markets?vs_currency=inr&ids=${targetIds.join(",")}&price_change_percentage=1h,24h,7d`).then((r) => (r.ok ? r.json() : null))
            ...
            const cgHoldingsData = cgHoldingsResult && cgHoldingsResult.status === "fulfilled"
                ...
                dcxData.forEach((d) => {
                    if (d.symbol && d.current_price > 0) {
                        ...
                        livePrices[sym] = {
                            ...
                            change: d.price_change_percentage_24h !== undefined
        ...
        if (currentUser && currentUser.loggedIn && currentUser.has_coindcx) {
            ...
            if (currentUser.coindcx_sync_status === "syncing") {
                ...
                const lastSynced = currentUser.coindcx_last_synced
                    ...
                    : "Just now";
                if (text)
                    text.textContent = `Synced at ${lastSynced}`;
            }
        ...
        syncInterval = setInterval(async () => {
            ...
            if (!currentUser || currentUser.coindcx_sync_status !== "syncing") {
                ...
                if (currentUser && currentUser.coindcx_sync_status === "success") {
                    ...
                    showToast("CoinDCX sync failed: " +
                        (currentUser.coindcx_sync_error || "Unknown error"), "err");
                }
                await fetchHoldings();
                await fetchTrades();
                await fetchLivePrices();
  ...
  `;
    ...
    function resetPortfolioPwFields() {
        ...
        if (dcxKey)
            dcxKey.type = "password";
        if (dcxSecret)
            dcxSecret.type = "password";
    ...
    async function saveKeys() {
        ...
        const secret = document.getElementById("dcxSecret").value.trim();
        if (!key)
            return showToast("Please enter API key.", "err");
        ...
        try {
            ...
            if (!res.ok) {
                ...
                showToast(data.error ||
                    data.message ||
                    `Failed to trigger sync (HTTP ${res.status}).`, "err");
                return;
            }
            await fetchUser();
  ...
  `;
    ...
    function renderHoldingsTable(list, q) {
        if (q)
            list = list.filter((h) => h.sym.toLowerCase().includes(q) || h.name.toLowerCase().includes(q));
  ...
  `;
      ...
      <tr>
        ...
        tbody.innerHTML = list
            ...
            const imgUrl = window.COINGECKO_LOGO_MAP[sym] ||
    ...
    function renderDistributions(enriched, q) {
        if (q)
            enriched = enriched.filter((h) => h.sym.toLowerCase().includes(q) || h.name.toLowerCase().includes(q));
        // Sort by value
  ...
  `)
        ...
        basePrices.forEach((point, idx) => {
            ...
            results.forEach((res) => {
                ...
                if (res.prices[idx]) {
                    ...
                    const nearest = res.prices.reduce((prev, curr) => Math.abs(curr[0] - ts) < Math.abs(prev[0] - ts) ? curr : prev, res.prices[0]);
    ...
    function initRealtimeUpdates() {
        ...
        socket.on("tickerUpdate", (data) => {
            ...
            if (row) {
                ...
                if (balanceValueEl)
                    balanceValueEl.textContent = "≈ " + formatMoney(value);
                if (h) {
                    const avg = h.buy_price > 0 ? h.buy_price : null;
    ...
    window.openAddEditHoldingForMissing = openAddEditHoldingForMissing;
    window.openEditHoldingModal = openEditHoldingModal;
    window.delHolding = delHolding;
}
