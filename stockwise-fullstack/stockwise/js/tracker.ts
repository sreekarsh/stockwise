...
(function () {
  ...
  function showShimmer() {
    ...
    if (cg && cg.innerHTML.trim() === "") {
      ...
      if (cg) cg.classList.add("loading");
    }
  }
    ...
    try {
      ...
      if (currency === "inr" || currency === "usdt" || currency === "usd") {
        const dcxParams = new URLSearchParams({
          vs_currency:
            currency === "usdt" ? "usdt" : currency === "usd" ? "usd" : "inr",
        });
      ...
      if (dcxRes) {
        try {
          ...
          if (Array.isArray(dcxData)) {
            const dcxMap = {};
            dcxData.forEach((d) => {
              if (d.symbol) dcxMap[d.symbol.toUpperCase()] = d;
            });
              ...
              if (dcx && dcx.current_price != null) {
                ...
                return {
                  ...
                  price_change_percentage_24h:
                    dcx.price_change_percentage_24h ??
  ...
  function updateCategoryBar(type) {
    ...
    const cc = $("cryptoCats");
    if (cc) cc.style.display = type === "crypto" ? "inline-flex" : "none";
    const sc = $("stockCats");
    if (sc) sc.style.display = type === "stock" ? "inline-flex" : "none";
  }
  ...
  function toggleFavMode() {
    favMode = !favMode;
    const el = $("favToggle");
    if (el) el.classList.toggle("active", favMode);
    filterTable();
  }

  function updateWatchlistCount() {
    const el = $("sStatWatch");
    if (el) el.textContent = favSet.length;
  }
  ...
  function toggleFav(sym, btnEl) {
    ...
    allBtns.forEach((b) => {
      ...
      if (path) path.setAttribute("fill", isFaved ? "#ffd43b" : "currentColor");
    });
  ...
  async function fetchBuyPricesForCoins(coinsSlice) {
    const currency = $("currencySelect")?.value || "usd";
    if (currency !== "inr" && currency !== "usdt" && currency !== "usd") return;
    ...
    try {
      const buyRes = await fetch(
        `/api/coindcx/buy-prices?${buyParams.toString()}`,
        { credentials: "include" },
      );
  ...
  function renderTable(coins, totalCount = coins.length) {
    ...
    grid.innerHTML = coins
      .map((c) => {
        ...
        const sign24 = h24 > 0 ? "+" : "";
        const clsPill =
          h24 === 0 ? "ct-neutral" : h24 > 0 ? "ct-up" : "ct-down";
        ...
        const img =
          (window.COINGECKO_LOGO_MAP && window.COINGECKO_LOGO_MAP[sym]) ||
          (c.image
            ? escAttr(c.image)
        ...
        const chg24Cls =
          h24 === 0 ? "neutral" : h24 > 0 ? "positive" : "negative";
    ...
    const cc = $("coinCount");
    if (cc) cc.textContent = `${totalCount} assets`;
  }
  ...
  async function loadChart() {
    ...
    try {
      ...
      if (!isStock && prices.length > 0) {
        const currentCoin = allCoins.find(
          (c) =>
            (c.id && c.id === lastQueriedChartCoin) ||
      ...
      if (activeIndicators.sma && sma20)
        indText.push(
          `SMA(${indicatorPeriods.sma}): ${sma20.at(-1)?.toFixed(2) ?? "N/A"}`,
        );
      if (activeIndicators.ema && ema20)
        indText.push(
          `EMA(${indicatorPeriods.ema}): ${ema20.at(-1)?.toFixed(2) ?? "N/A"}`,
        );
      if (activeIndicators.rsi && rsi)
        indText.push(
          `RSI(${indicatorPeriods.rsi}): ${rsi.at(-1)?.toFixed(1) ?? "N/A"}`,
        );
  ...
  function openAlertModal(sym, price) {
    ...
    if (sl) sl.textContent = sym;
    const ap = $("alertPrice");
    if (ap) ap.value = price != null ? Number(price).toFixed(3) : "";
    const ae = $("alertErr");
  ...
  async function saveAlert() {
    ...
    if (data.success) {
      closeAlertModal();
      window.stockwise?.toast(
        `Alert set for ${currentAlertSymbol} 🔔`,
        "success",
      );
    } else {
  ...
  async function loadProfileSidebar() {
    try {
      ...
      if (pn) pn.textContent = me.username || "User";
      const pe = $("profileEmail");
      if (pe) pe.textContent = me.email || "";
      ...
      if (sp) sp.textContent = portfolios.length;
      const sa = $("sStatAlerts");
      if (sa) sa.textContent = alerts.length;
      const sw = $("sStatWatch");
      if (sw) sw.textContent = favSet.length;
      if (me.created_at) {
        const days = Math.max(
          1,
          Math.floor((Date.now() - new Date(me.created_at)) / 864e5),
        );
        const el = $("sStatDays");
  ...
  function initRealtimeUpdates() {
    ...
    socket.on("stockUpdates", (updates: Array<{ symbol: string; current_price: number; price_change_percentage_24h: number }>) => {
      if (!Array.isArray(updates)) return;
      for (const update of updates) {
        const stock = allStocks.find((s: any) => s.symbol === update.symbol);
        if (!stock) continue;
  ...
  async function boot() {
    ...
    const gridBtn = $("layoutGridBtn");
    const listBtn = $("layoutListBtn");
    if (gridBtn) gridBtn.classList.toggle("active", currentViewMode === "grid");
    ...
    const initType =
      p.get("type") === "stock" || p.get("type") === "crypto"
        ? p.get("type")
        : "all";
