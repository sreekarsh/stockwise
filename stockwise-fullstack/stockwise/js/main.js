...
(function () {
    ...
    function onDown(e) {
        ...
        if (!hdr || e.target.closest(".ai-close"))
            return;
        const win = document.getElementById("aiWindow");
        if (!win.classList.contains("open"))
            return;
...
function askAI(query) {
    ...
    const wantsEndpoint = /(endpoint|api|route|url|how to call|request)/i.test(lower);
    const wantsMl = /(ml|machine learning|model|predict|regime|sentiment|shap)/i.test(lower);
    const wantsHowTo = /(how|guide|steps|run|setup|deploy|install|start)/i.test(lower);
    appendAiMsg("bot", "Thinking…");
    setTimeout(() => {
        // If user is asking for platform help, keep it contextual.
        let response = "I can help with StockWise features like Live Tracker, AI Signals, Portfolio Sync, Analyzer, and Bot Trading.";
        ...
        else if (/(ai signals|signals)/i.test(lower) ||
        ...
        if (chat) {
            ...
            if (last &&
                ...
                if (botEl)
                    botEl.style.whiteSpace = "pre-line";
            }
        }
    }, 500);
}
...
function loadRememberedLogin() {
    ...
    if (remembered.email) {
        ...
        if (regEmail)
            regEmail.value = remembered.email;
...
async function loadTicker() {
    try {
        ...
        if (!Array.isArray(coins) || coins.error)
            return;
        tickerCache = coins;
        tickerCacheTime = now;
        renderTicker(coins);
    ...
    try {
        ...
        const rows = Object.entries(data)
            ...
            const marketCap = payload.usd_market_cap
                ...
                : "N/A";
            const change24h = payload.usd_24h_change !== undefined
...
function renderBotTip(text) {
    if (_botBubble)
        _botBubble.querySelector(".bot-msg").textContent = text;
}
  ...
  `;
    btn.onclick = () => {
        ...
        if (_botOpen)
            ...
            btn.style.animation = "botPulse 2.5s ease-in-out infinite";
    };
...
function dismissGuideBot() {
    ...
    if (_botEl)
        _botEl.style.animation = "botPulse 2.5s ease-in-out infinite";
}
...
function onPasswordInput() {
    ...
    rules.forEach((rule) => {
        ...
        if (ok)
            passed++;
        rule.className = "pw-rule" + (ok ? " pass" : "");
        rule.querySelector(".pw-icon").textContent = ok ? "✓" : "○";
    });
    ...
    if (pct === 0) {
        ...
        bar.style.background = "#00e5a0";
        bar.textContent = "Strong";
    }
}
function toggleRegPw(btn) {
    const input = document.getElementById("regPass");
    if (!input)
        return;
...
window.sendResetRequest = sendResetRequest;
window.completePasswordReset = completePasswordReset;
window.suggestPassword = suggestPassword;
